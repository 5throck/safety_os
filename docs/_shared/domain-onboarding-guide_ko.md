...
본 가이드는 2026-06-17에 채택된 폴더 구조 규칙(회의 `memory/meeting-2026-06-17-folder-structure-redesign.md` 기준)을 문서화합니다.
...
모든 도메인은 일관된 `<top-level>/domains/<name>/` 패턴으로 다음 최상위 디렉토리에 접합니다:
...
evidence-models/
├── _shared/base/                  # 공통 스키마 (gmp-common, finding 등)
└── domains/
    └── <name>/
        └── <name>-*-record.json   # 도메인별 증거 모델

skills/
├── _meta/                         # SKILLS.md, README 파일
...
### Step 2: 표준 하위 구조 생성
각 최상위 디렉토리에 대해 `domains/<name>/` 하위 디렉토리를 생성합니다:
...
### Step 3: 에이전트 작성
`_core/` 에이전트의 템플릿에 따라 `agents/domains/<name>/<name>-agent.md`를 작성합니다. 필수 섹션:
- Section A — Legal Basis (다중 출처; 최소 2개 규제 참조)
...
### Step 5: 워크플로우 작성
각 워크플로우에 대해:
1. `workflows/domains/<name>/<workflow>/` 디렉토리 생성
...
### Step 6: 증거 모델 작성
각 증거 유형에 대해 `evidence-models/domains/<name>/<name>-*-record.json`을 작성합니다. (도메인 결정에 따라 적용 가능한 경우) 필수 공통 필드:
...
- `legal_basis` (다중 출처 배열, 최소 2개 항목)

### Step 7: 스킬 작성 (선택)
도메인이 횡단적 방법론 스킬(예: `gmp-qrm`)을 가진 경우 `skills/domains/<name>/<skill>/SKILL.md`를 작성합니다.

### Step 8: 규제 참조 작성
감사 준수를 위해 `source_mcp` 필드를 포함하여 `regulations/<jurisdiction>/<Regulator>-<Framework>.{yaml,md}`를 작성합니다.

### Step 9: 범위 문서 작성
v1 범위, 제외 항목, KPI, 컴플라이언스 게이트를 문서화하는 `docs/domains/<name>/scope.md`를 작성합니다.
...
2. **증거 모델**: `evidence-models/<prefix>-*.json`에서 `evidence-models/domains/<name>/<prefix>-*.json`로 이동. `$ref` 경로를 `base/X.schema.json`에서 `../../_shared/base/X.schema.json`로 갱신.
3. **에이전트 참조 갱신**: 에이전트 `.md` 파일은 워크플로우 경로를 참조합니다. `workflows/<name>/X.md`에서 `workflows/domains/<name>/X/README.md`로 갱신.
4. **스킬 참조 갱신**: 스킬 파일은 에이전트 경로를 참조할 수 있습니다. `agents/X-agent.md`에서 `agents/domains/<name>/X-agent.md`(또는 `agents/_shared/X-agent.md`)로 갱신.

## 7. 검증 체크리스트

- [ ] 모든 도메인 파일이 `<top-level>/domains/<name>/` 하위에 위치
