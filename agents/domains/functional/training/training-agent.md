...
2. **Gap Analysis**: Compare existing `training-record.json` `completion_date`, `hours_completed` vs `required_hours`, and `next_training_due` fields against OSHA-KR Article 29 (제29조) requirements, validating every record's `legal_basis` array against `regulations/KR/legal-glossary.yaml`.
