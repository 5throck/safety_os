...
**KOSHA GUIDE Z-40-2022** ("LOTO에 관한 지침", 한국산업안전보건공단/KOSHA, 2022) is also referenced as a Korean-specific technical guideline. This citation was provided by the project owner; its full text could not be independently retrieved or verified in this session (the connected legal MCPs — `mcp_kr_legislation`, `legalize_kr`, `k_skill` — do not index KOSHA's own technical guide series, and a web fetch attempt failed due to a tool/model error, not a URL problem). Because its content has not been read, no specific provisions from it have been incorporated into this workflow's design yet — it is listed here as a known, named source to consult when the specific 안전보건규칙 article and detailed KOSHA-recommended procedure are confirmed by a qualified safety professional.

Confirmation of the exact Korean 안전보건규칙 article, and review of KOSHA GUIDE Z-40-2022's full content, remain open items requiring review by a qualified safety professional before operational use, per this project's Legal Disclaimer.
