...
metadata:
  ...
  agents:
    - compliance-agent
  legal_basis:
    - ?곗뾽?덉쟾蹂닿굔踰??꾨컲 (Occupational Safety and Health Act ??all articles)
    - 以묐??ы빐泥섎쾶踰??꾨컲 (Serious Accidents Punishment Act ??all articles)
scope: workspace
---
...
1. **Regulation Identification** ??Identify all applicable regulations based on the industry sector, workplace type, number of employees, and work activities. Key frameworks:
   - ?곗뾽?덉쟾蹂닿굔踰?(OSHA-KR) and subordinate regulations
   - 以묐??ы빐泥섎쾶踰?(SAPA) ??applies to workplaces with 5+ employees
   - ?뷀븰臾쇱쭏愿由щ쾿 (Chemical Substances Control Act) ??if applicable
   - 怨좎븬媛???덉쟾愿由щ쾿 ??if applicable
...
4. **Corrective Action Recommendation** ??For each identified gap, recommend a corrective action with estimated effort, responsible party, and target completion date. Prioritize Critical gaps (those with potential criminal liability under 以묐??ы빐泥섎쾶踰? first.
...
|---|---------|-------------|---------------|-----|------------|
| 1 | ??9議? | Training records for all workers | Partial records only | Missing records for 15 workers | Major |
...
- ?곗뾽?덉쟾蹂닿굔踰?imposes duties on employers across all aspects of occupational safety and health. Violations can result in fines up to 50 million KRW or imprisonment.
- 以묐??ы빐泥섎쾶踰?applies criminal penalties (up to 1 year imprisonment or 1 billion KRW fine for the business) when serious accidents result from failure to meet safety management obligations.
